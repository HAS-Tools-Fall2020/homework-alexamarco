...
q_pred_test = model.intercept_ + model.coef_[0] * test['flow_tm1'] + model.coef_[1] * test['flow_tm2'] + model.coef_[2] * test['flow_tm3'] 
...
ax.set(title = 'Scatterplot and AR Model With Log Axes', xlabel='flow t minus shift', ylabel='flow t', yscale='log', xscale='log')
ax.plot(np.sort(train['flow']), np.sort(q_pred), label='AR model', color = 'firebrick')
ax.legend()
fig.set_size_inches(10,6)
