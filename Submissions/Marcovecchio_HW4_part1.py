...
# Use the linspace  funciton to create a set  of evenly spaced bins
# another example using the max flow to set the upper limit for the bins
mybins5 = np.linspace(0, 1500, num=15) 
#Plotting the histogram
plt.hist(wk5arr, bins = mybins5)
plt.title('Week 5 Streamflow')
plt.xlabel('Flow [cfs]')
plt.ylabel('Count')
...
# Two different approaches ---  you should get the same answer
# just using the flow column
flow_quants5 = np.quantile(wk5arr, q=[0,0.1, 0.5, 0.9])
print("Week 5 Quants", flow_quants5)
flow_quants6 = np.quantile(wk6arr, q=[0,0.1, 0.5, 0.9])
print("Week 6 Quants", flow_quants6)
#print('Method one flow quantiles:', flow_quants1)
# Or computing on a colum by column basis 
#flow_quants2 = np.quantile(flow_data, q=[0,0.1, 0.5, 0.9], axis=0)
# and then just printing out the values for the flow column
...
# %%
plt.close()
mybins6 = np.linspace(0, 1500, num=15) 
plt.hist(wk6arr, bins = mybins6)
plt.title('Week 6 Streamflow')
plt.xlabel('Flow [cfs]')
plt.ylabel('Count')
# %%
