...
for i in range(rows-1):
        if flow_data[i,0] >= 2010:
                if (flow_data[i,1] == 8 and flow_data[i,2] == 31) or (flow_data[i,1] == 9 and flow_data[i,2] >= 1 and flow_data[i,2] <= 6):
                        ct2 = ct2 + 1
...
for j in range(ct2-1):
        if week2prediction < wk2arr[j]:
                count1 = count1 + 1

for k in range(ct3-1):
        if week3prediction < wk3arr[k]:
                count2 = count2 + 1

for m in range(ct4-1):
        if week4prediction < wk4arr[m]:
                count3 = count3 + 1

for n in range(ct5-1):
        if week5prediction < wk5arr[n]:
                count4 = count4 + 1

for o in range(ct6-1):
        if week6prediction < wk6arr[o]:
                count5 = count5 + 1

count = count1 + count2 + count3 + count4 + count5
tot = wk2arr.size + wk3arr.size + wk4arr.size + wk5arr.size + wk6arr.size
print(count)
print(count/tot)



#print("Normal 16 week forecast:",weeklist)
